...
def Matrixproductotensor(m1,m2):
    c=[]
    for j in range(len(m1)):
        res=multescalarmatrices(m1[j],m2)
        for k in range(len(res)):
            c.append(res[k])
    return c
...
def norma(m1):
    if tuple == type(m1[0]):
        return ((innervector(m1,conjugadavector(m1)))[0])**(1/2)
    else:
        return ((((innermatrix(m1,adjmatriz(m1)))))[o])**(1/2)

def productotensor(m1,m2):
    c=[]
    for i in range(len(m1)):        
        for j in range(len(m1)):
            res=multescalarmatrices(m1[i][j],m2)
            for k in range(len(res)):
